...
class SnakeGame:
    ...
    def _move(self, action):
        directions_list = [Direction.RIGHT, Direction.DOWN, Direction.LEFT, Direction.UP]
        index = directions_list.index(self.direction)

        if np.array_equal(action, [1, 0, 0]):
            new_direction = directions_list[index]
        elif np.array_equal(action, [0, 1, 0]):
            new_direction = directions_list[(index + 1) % 4]
